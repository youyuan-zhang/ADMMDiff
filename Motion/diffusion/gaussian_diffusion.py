...
class GaussianDiffusion:
    ...
    def __init__(self, conf: DiffusionConfig):
        ...
        self.atbar = torch.tensor(self.alphas_cumprod, device="cuda")
        self.atbar_prev = torch.tensor(self.alphas_cumprod_prev, device="cuda")
        self.at = torch.tensor(self.alphas, device="cuda")
        self.beta_tilde = torch.tensor(self.posterior_variance, device="cuda")
        self.beta_tilde[0] = 0
    ...
    def condition_mean_with_grad(self,
        ...
        print(gradient.square().sum().sqrt())
    ...
    def p_sample_loop_admm(
        ...
        device = "cuda"
        # xt = th.randn(*shape, device=device)
        indices = list(range(self.num_timesteps - skip_timesteps))[::-1]
        from tqdm.auto import tqdm
        indices = tqdm(indices)
        ...
        previous_xstart = xt
        # params
        nu = torch.zeros_like(zt) # dual variable
        # nu = torch.nn.Parameter(nu.to(x.device))
        # optim_nu = torch.optim.Adam([nu], lr=0.01)
        max_iter, c = 20, 1.0 # steps SGD
        
        
        # def loss_cond(x):
        #     sample = transform_fn(x, t2m_dataset)
        #     cond = (sample[:, 0, :, 0] - sample[:, 0, :, -1]).square().sum().sqrt()
        #     return cond
        ...
        for i in indices:
            t = th.tensor([i] * shape[0], device=device)
            with th.no_grad():
                # rho = c * (np.sqrt(self.at[i].item()) * (1 + np.sqrt(self.beta_tilde[i].item()) * np.sqrt(self.at[i].item())
                #                                          / np.sqrt((1 - self.at[i]).item()))) / (1 - self.at[i]).item()
                rho = c
                xt = zt - nu / rho
                pass
            ...
            with torch.enable_grad():
                ...
                pred_x0 = model(xt, self._scale_timesteps(t), **model_kwargs)
                s = (torch.sqrt(self.atbar[i]) * pred_x0 - xt) / (1 - self.atbar[i])
                pred_xt = (xt + (1 - self.at[i]) * s) / torch.sqrt(self.at[i])
                
                if cond_fn is not None and i > 950:  # 950
                    gradient = cond_fn(xt, t, {"pred_xstart": pred_x0}, **model_kwargs)
                    if i <= 1000: gradient = gradient * 1
                    print(gradient.square().sum().sqrt())
                    pred_xt = (pred_xt.float() + self.beta_tilde[i] * gradient.float())
                
            noise = th.randn_like(xt)
            xt = pred_xt + (t != 0).float().view(-1, *([1] * (len(xt.shape) - 1))) * self.beta_tilde[i] * noise
            ...
            for k in range(max_iter):
                with torch.enable_grad():
                    ...
                    pred_z0 = (zt + (1 - self.atbar[i]) * s) / torch.sqrt(self.atbar[i])
                    if cond_fn is not None and i >= 20:
                        gradient = cond_fn(zt, t, {"pred_xstart": pred_z0}, **model_kwargs)
                        print(gradient.square().sum().sqrt())
                        if i > 900:  # 900
                            gradient = gradient * 0.00000
                        else:
                            gradient = gradient * 0.5
                    zt = zt + 0.1 * (rho * (pred_xt - zt) + nu) + self.beta_tilde[i] * gradient.float()
                zt = zt.detach()
                
                print(f"timestep: {i}, xtzt diff: {((xt-zt)**2).sum().item()}")
                
                
                
                # if cond_fn is not None and i >= 20:
                #     gradient = cond_fn(z0, t, {"pred_xstart": z0}, **model_kwargs)
                #     if i <= 1000: gradient = gradient * 1
                #     print(gradient.square().sum().sqrt())
                #     z0 = z0 - 0.01 * gradient
                #     z0 = z0.detach()
            ...
            #             # Need to compute x_{t-1} from x_start again
            #             model_mean_imputed, _, _ = self.q_posterior_mean_variance(
            #                 x_start=imputed_xstart, x_t=xt.detach(), t=t)
            #             # Combine inpainted sample with grad sample. Ratio is arbitrary
            #             # Combine means should be better than combine samples with noises
            ...
            #             # Combine inpainted sample with grad sample. Ratio is arbitrary
            #             xt = combined_mean + ((t != 0).float().view(-1, *([1] * (len(xt.shape) - 1)))) * th.sqrt(self.beta_tilde[i]) * noise
            
            # z_old = torch.sqrt(self.atbar[i]) * z0 - (1 - self.atbar[i]) * s
            # z_old = (z_old + (1 - self.at[i]) * s) / torch.sqrt(self.at[i]) + torch.sqrt(self.beta_tilde[i]) * noise
            
            # z0 = torch.nn.Parameter(z0.to(device))
            # optimizer = torch.optim.Adam([z0], lr=0.01)
            # loss_prev = 1e10
            
            # for k in range(max_iter):
            #     optimizer.zero_grad()
            #     zt = torch.sqrt(self.atbar[i]) * z0 - (1 - self.atbar[i]) * s
            #     zt = (zt + (1 - self.at[i]) * s) / torch.sqrt(self.at[i]) + torch.sqrt(self.beta_tilde[i]) * noise
                
            #     loss =  (nu * (xt-zt)).sum() + 0.5 * rho * ((xt - zt) ** 2).sum() - 0 * loss_cond(zt)
            #     loss.backward()
            #     optimizer.step()
            #     print(f"timestep: {t}, xt_cond: {loss_cond(xt).item()}, zt_cond: {loss_cond(zt).item()}, xtzt diff: {((xt-zt)**2).sum().item()}")
            
            # nu = nu + rho * (xt.data - zt.data)
        
            # rk = (xt - zt).square().sum().sqrt()  # primal
            # sk = rho * (zt - z_old).square().sum().sqrt() # dual
            # if rk > 10 * sk:
            #     c = 2.0 * c
            # elif sk > 10 * rk:
            #     c = c / 2.0
            ...
            #     if cond_fn is not None and int(t[0]) >= 20:
            #         # you need to supply with "unaltered x0" and calculate the gradient from it.
            #         # but you must only update mean in the "inpainting region" not outside (which will interfere with the imposing region).
            #         out["mean"] = self.condition_mean_with_grad(
            #             cond_fn,
            ...
            if self.log_trajectory_fn is not None:
                ...
                if int(t[0]) in out_list:
                    self.log_trajectory_fn(pred_x0.detach(),
                                           model_kwargs['y']['log_name'], out_list,
                                           t, model_kwargs['y']['log_id'])
            ...
            if i < 600: return[pred_x0.detach()]
