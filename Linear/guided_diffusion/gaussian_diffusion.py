...
class GaussianDiffusion:
    ...
    def p_sample_loop_admm(self,
        ...
        pbar = tqdm(list(range(self.num_timesteps))[::-1])
        for idx in pbar:
            t = torch.tensor([idx] * xt.shape[0], device=device)
            ...
            # TODO: how can we handle argument for different condition method?
            pred_xt, distance, norm_grad = measurement_cond_fn(x_t=pred_xt,
                                      measurement=measurement,
                                      noisy_measurement=noisy_measurement,
                                      x_prev=xt,
                                      x_0_hat=pred_x0)
            ...
            if idx != 0:  # no noise when t == 0
                xt = pred_xt + torch.exp(0.5 * out['log_variance']) * noise

            xt = xt.detach()

            if record:
                if idx % 10 == 0:
                    file_path = os.path.join(save_root, f"progress/x.png")
                    plt.imsave(file_path, clear_color(pred_x0))
            ...
            for k in range(max_iter):
                ...
                zt = zt.detach()
                # print(f"timestep: {idx}, xtzt diff: {((pred_xt-zt)**2).sum().item()}")
            
            zt = zt + torch.exp(0.5 * out['log_variance']) * noise
